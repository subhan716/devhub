...
import { TerminalSquare, Mail, Lock, User, ArrowRight, Sun, Moon } from 'lucide-react';
...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Set global form error if it's a known backend conflict (e.g. email exists)
      if (errorMsg.toLowerCase().includes('already exists')) {
        setErrors({ ...errors, email: 'This email is already registered to another developer.' });
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0A0A0A] flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden transition-colors duration-200">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <h2 className="text-center text-3xl font-extrabold text-slate-900 dark:text-white">Join the Network</h2>
        <p className="mt-2 text-center text-sm text-slate-600 dark:text-gray-400">
          Already have an account?{' '}
          <Link to="/login" className="font-medium text-[#0A66C2] dark:text-[#00F0FF] hover:underline transition-colors">
            Sign in here
          </Link>
        </p>
      </div>
      ...
      >
        <div className="bg-white dark:bg-white/[0.02] py-8 px-4 shadow-2xl border border-slate-200 dark:border-white/10 sm:rounded-2xl sm:px-10 backdrop-blur-xl">
          <form className="space-y-6" onSubmit={handleSubmit} noValidate>
            <div>
              <label className="block text-sm font-semibold text-slate-700 dark:text-gray-300">Developer Name</label>
              <div className="mt-1 relative rounded-md shadow-sm">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <User className={errors.name ? "text-red-500" : "text-gray-500"} size={18} />
                </div>
                <input
                  name="name"
                  type="text"
          ...
          <div className="mt-6">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white dark:bg-[#0A0A0A] text-slate-500 dark:text-gray-500">Or continue with</span>
              </div>
            </div>
            <div className="mt-6 grid grid-cols-2 gap-4">
              <button
                onClick={() => window.location.href = `${import.meta.env.VITE_API_URL}/api/auth/google?intent=register`}
                className="w-full flex justify-center items-center gap-2 py-3 px-4 border border-slate-300 dark:border-white/10 rounded-xl bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 text-sm font-semibold text-slate-800 dark:text-white shadow-xs transition-all cursor-pointer"
              >
